...
final class SiteIntegration {
    ...
    static String install(String downloadLabel) {
        return "(function(){" +
                ...
                "function episodeDownloadElement(){if(!/^\\/media\\/[^/]+\\/\\d+\\/?$/.test(location.pathname))return null;var all=[].slice.call(document.querySelectorAll('a,button,[role=button]'));return all.find(function(e){if(e.closest('[data-animeav1-app-nav]'))return false;return label(e).indexOf('descarg')===0;})||null;}" +
