...
final class SiteIntegration {
    ...
    static String install(String downloadLabel) {
        ...
        return "(function(){window.__animeav1AndroidVersion=" + version + ";window.__animeav1DownloadLabel=" + label + ";" + """
                ...
                function cleanPath(){var p=location.pathname||'/';while(p.length>1&&p.endsWith('/'))p=p.slice(0,-1);return p||'/';}
                function isTopLevel(){var p=cleanPath();return p==='/'||p==='/horario'||p==='/cuenta/listas'||p==='/cuenta';}
                ...
                function episodeParts(){var p=location.pathname.split('/').filter(Boolean);if(p.length===3&&p[0]==='media'&&/^[0-9]+$/.test(p[2]))return {slug:p[1],episode:parseInt(p[2],10)};return null;}
                ...
                function publishedCount(slug){var max=0;[].slice.call(document.querySelectorAll('a[href]')).forEach(function(a){try{var u=new URL(a.getAttribute('href'),location.href),p=u.pathname.split('/').filter(Boolean);if(p.length===3&&p[0]==='media'&&p[1]===slug&&/^[0-9]+$/.test(p[2]))max=Math.max(max,parseInt(p[2],10)||0);}catch(e){}});return max;}
