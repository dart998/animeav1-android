...
final class DownloadsIntegration {
    ...
    static String render(List<EpisodeStore.DownloadRecord> records) {
        ...
        return "(function(){" +
                ...
                "if(header){header.style.setProperty('position','relative','important');header.style.setProperty('z-index','2147483200','important');}" +
                "if(nav){nav.style.setProperty('z-index','2147483300','important');}" +
                ...
                "if(nav){var dl=nav.querySelector('[data-animeav1-downloads-link]');if(dl)dl.style.setProperty('color','#35dccd','important');}" +
                "window.__animeav1DownloadsOpen=true;" +
                "})()";
    }

    static String close() {
        return "(function(){var p=document.getElementById('animeav1-downloads-page');if(p)p.remove();window.__animeav1DownloadsOpen=false;})()";
    }
}
