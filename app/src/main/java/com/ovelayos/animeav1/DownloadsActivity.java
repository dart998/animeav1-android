...
public class DownloadsActivity extends Activity {
    ...
    private void buildUi(){getWindow().setStatusBarColor(BG);getWindow().setNavigationBarColor(BG);LinearLayout root=new LinearLayout(this);root.setOrientation(LinearLayout.VERTICAL);root.setBackgroundColor(BG);root.setPadding(dp(16),dp(12),dp(16),dp(12));
        ...
        root.addView(text("Gestiona tus episodios descargados, en cola y en curso.",14,MUTED,false));tabs=new LinearLayout(this);tabs.setPadding(0,dp(16),0,dp(8));root.addView(tabs,new LinearLayout.LayoutParams(-1,-2));ScrollView scroll=new ScrollView(this);content=new LinearLayout(this);content.setOrientation(LinearLayout.VERTICAL);content.setPadding(0,dp(6),0,dp(40));scroll.addView(content);root.addView(scroll,new LinearLayout.LayoutParams(-1,0,1));setContentView(root);}
    ...
    private View card(EpisodeStore.DownloadRecord r){LinearLayout c=new LinearLayout(this);c.setOrientation(LinearLayout.VERTICAL);c.setPadding(dp(14),dp(12),dp(14),dp(12));c.setBackgroundColor(CARD);LinearLayout.LayoutParams cp=new LinearLayout.LayoutParams(-1,-2);cp.setMargins(0,0,0,dp(10));c.setLayoutParams(cp);String name=(r.title==null||r.title.isEmpty())?r.slug:r.title;TextView t=text(name,16,Color.WHITE,true);t.setSingleLine(true);c.addView(t);c.addView(text("Episodio "+r.episode,14,MUTED,false));
    ...
    private void play(EpisodeStore.DownloadRecord r){Intent i=new Intent(this,OfflinePlayerActivity.class);i.putExtra(OfflinePlayerActivity.EXTRA_PATH,r.path);i.putExtra(OfflinePlayerActivity.EXTRA_TITLE,r.title.isEmpty()?r.slug:r.title);i.putExtra(OfflinePlayerActivity.EXTRA_EPISODE,r.episode);startActivity(i);}
